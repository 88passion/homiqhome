...
export const metadata = {
  title: "ซื้ออสังหาริมทรัพย์ | homiqhome",
  description: "รายการอสังหาริมทรัพย์สำหรับขาย บ้าน คอนโด ที่ดิน ทาวน์โฮม",
};
...
export default async function BuyPage({ searchParams }: PageProps) {
  ...
  const provinces = await getPublishedPropertyProvincesServer("buy");
  const districts = await getPublishedPropertyDistrictsServer("buy", filters.province ? filters.province : undefined);
    ...
    <main className="min-h-screen bg-white">
      <div className="mx-auto max-w-7xl px-4 py-6 md:px-6 md:py-10 lg:px-8">
        <div className="flex items-start justify-between gap-4 border-b border-black/5 pb-5 md:items-end md:pb-6">
          <div>
            <p className="text-xs font-medium uppercase tracking-[0.16em] text-black/45">Buy</p>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-black/70 md:text-base">
              เลือกดูบ้าน คอนโด ที่ดิน และทาวน์โฮมที่ทีมงานคัดไว้แล้ว พร้อมตัวกรองที่ช่วยให้หาได้เร็วขึ้น
            </p>
          </div>
          <Link
            href="/contact"
