...
export function getAllThaiProvinces(): string[] {
  return THAI_PROVINCES;
}

export function getDistrictsByProvince(province?: string | null): string[] {
  if (!province) return [];
  return THAI_DISTRICTS_BY_PROVINCE[province] ?? [];
}

export function sortProvincesByPriority(provinces: string[], priorityBoost?: string[]): string[] {
  const unique = Array.from(new Set(provinces));
  const boosted = priorityBoost ?? [];

  return unique.sort((a, b) => {
    const aPriority = PROVINCE_PRIORITY.indexOf(a as (typeof PROVINCE_PRIORITY)[number]);
    const bPriority = PROVINCE_PRIORITY.indexOf(b as (typeof PROVINCE_PRIORITY)[number]);
    ...
    const aBoost = boosted.indexOf(a);
    const bBoost = boosted.indexOf(b);
